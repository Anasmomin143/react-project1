...
export default function SignUp() {
  ...
  const handelsubmit = (event) => {
    ...
    if (newrecords.name === "" || newrecords.email === "" || newrecords.password === "" || newrecords.confirmpass === "") {
      alert("Please Enter value To SingUp")
    } else {
      navigatetoUser(`/user/${newrecords.name}`)
    }
  }
  return (
    <>
      ...
      <form onSubmit={handelsubmit}>
        <div className='container'>
          <div className="mb-3">
            ...
            <input type="text" name='UserName' className="form-control" id="UserName" value={name} onChange={NameChanger} />
          </div>
          <div className="mb-3">
          ...
          <div className="mb-3">
            ...
            <input type="password" name='UserConfirmPass' className="form-control" id="UserConfirmPassword" value={confirmpass} onChange={ConfirmPassChanger} />
          </div>
          <button type="submit" className="btn btn-primary" onClick={SignUpFunction}>Submit</button>
        </div>
      </form>
    </>
  )
